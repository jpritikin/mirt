                       ...
                       nominal.highlow = NULL, GenRandomPars = FALSE, ...)
{
    ...
    if(opts$method == 'EM'){
        ...
        ESTIMATE <- EM.group(pars=pars, constrain=constrain, Ls=Ls, PrepList=PrepList,
                             list = list(NCYCLES=opts$NCYCLES, TOL=opts$TOL, MSTEPTOL=opts$MSTEPTOL,
                                         ...
                                         customPriorFun=opts$customPriorFun, technical=opts$technical),
    ...
    if (is.null(ESTIMATE$mstepLogLik)) ESTIMATE$mstepLogLik <- 0
    mstepLogLik <- ESTIMATE$mstepLogLik
    r <- rr
    N <- sum(r)
    tmp <- dfsubtr
    AIC <- (-2) * logLik + 2 * tmp
